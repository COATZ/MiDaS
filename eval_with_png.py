...
parser.add_argument('--dataset',             type=str,   help='dataset to test on, nyu or kitti', default='RWFOREST')
parser.add_argument('--raw_crop',                        help='if set, crops according raw values', action='store_true')
...
def test():
    ...
    for root, dirnames, filenames in os.walk(args.pred_path):
        for pred_filename in fnmatch.filter(filenames, '*_dep_pred.png'):
            print(pred_filename)
    ...
    elif args.dataset == 'RWFOREST':
        for t_id in range(num_test_samples):
            file_dir = pred_filenames[t_id].split('.')[0]
            # print(file_dir)
            filename = file_dir.split('_pred')[0]
            # directory = file_dir.replace('_rgb', '_dep')
    ...
    silog, log10, abs_rel, sq_rel, rms, log_rms, d1, d2, d3 = eval(pred_depths)

    np.savetxt(args.pred_path+"res.csv", (d1, d2, d3, abs_rel, sq_rel, rms, log_rms, silog, log10), delimiter=",")
...
def eval(pred_depths):
    ...
    for i in range(num_samples):
        ...
        pred_depth = pred_depths_valid[i][:, :, 0]
        ...
        if args.garg_crop or args.eigen_crop:
            ...
            eval_mask[int(0.1 * gt_height):int(0.9 * gt_height), int(0.1 * gt_width):int(0.9 * gt_width)] = 1
            ...
            print("Going Eigen Crop")

        if args.raw_crop:
            height, width = gt_depth.shape
            eval_mask = np.zeros(valid_mask.shape)
            eval_mask[int(height*0.1):int(height*0.9), int(width*0.1):int(width*0.9)] = 1
            valid_mask = np.logical_and(valid_mask, eval_mask)
            # pred_depth_cropped = pred_depth[int(height*0.1):int(height*0.9), int(width*0.1):int(width*0.9)]
            # gt_depth_cropped = gt_depth[int(height*0.1):int(height*0.9), int(width*0.1):int(width*0.9)]
            # pred_depth = pred_depth_cropped
            # gt_depth = gt_depth_cropped
            # valid_mask = np.logical_and(gt_depth > args.min_depth_eval, gt_depth < args.max_depth_eval)
            # pred_depth_cropped = pred_depth[int(height*0.1):int(height*0.9), int(width*0.1):int(width*0.9)]*256.0
            # print(pred_depth_cropped)
            # # pred_depth_cropped = np.zeros((height, width), dtype=np.float32)
            # # pred_depth_cropped[int(height*0.1):int(height*0.9), int(width*0.1):int(width*0.9)] = pred_depth
            # print(pred_depth.shape)
            # print(pred_depth_cropped.shape)
            # cv2.imwrite("test.png", pred_depth_cropped)
            # print("Going Raw Crop")
            # sys.exit()
